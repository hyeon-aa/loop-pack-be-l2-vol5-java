...
- 고객 요청자는 `X-USER-ID`로 식별한다. 없는 사용자와 다른 사용자의 주문·좋아요 접근은 거절한다.
...
[PointBalance]   PointBalance Entity / Root
                 ├─ PointGrant Entity (개별 지급 건)
                 └─ PointUsage Entity (지급 건별 사용·만료 기록)
```
...
| PointBalance   | 전체 잔액, 충전·결제·만료·사용 출처 결정, 기록과 잔액의 일치             | 2% 보상 금액 계산, Repository·HTTP 응답 코드               |
| PointGrant     | 식별 가능한 충전/적립 지급 사실, 지급액·시각·만료 정보·적립 원인          | 수정되는 remainingAmount 저장                              |
| PointUsage     | 원천 지급 건별 결제·만료 금액과 시각 보존                                | 별도의 외부 변경 진입점                                    |
...
- **PT-05:** 최초 결제 후 `OrderConfirmationService`가 `floor(결제액 × 2 / 100)`을 계산해 PointBalance에 적립을 요청한다. 계산 중 overflow도 방지해야 하며, PointBalance는 지급 건·잔액·기록의 일치를 담당한다. 이번 적립분을 같은 결제에 선사용하지 않는다.
...
sequenceDiagram
    ...
    alt 이미 CONFIRMED
        ...
        alt 재고 또는 포인트 부족
            ...
            Confirm->>Confirm: rewardFor(totalAmount)
            Confirm->>Point: reward(reward)
...
아래는 설계 단계에서 정한 대표 기대값이며, 구현과 테스트를 검토할 때 사용한 기준이다.
...
- domain 모델 테스트는 재고·주문·포인트의 정상 처리와 대표 오류, 실패 후 상태 유지를 검증한다.
- application 테스트는 Repository 협력과 주문 확정 순서를 검증한다.
- JPA 통합 테스트는 flush/clear 후 재조회하여 주문 품목·포인트 지급 건·사용 기록·롤백을 검증한다.
- MockMvc 테스트는 고객과 관리자 API 계약, 관리자 헤더 경계를 검증한다.
- ArchUnit은 domain/application/interfaces의 금지 의존을, Checkstyle은 코드 스타일 규칙을 검증한다.
...
| 포인트 정책 객체 | 별도 Spend/Reward 정책 / 현재 위치               | 2% 보상 금액은 OrderConfirmationService, 지급 건 변경은 PointBalance | 계산이 복잡해지거나 여러 정책이 필요할 때 분리 검토                       |
...
- 주문 취소와 포인트 복원을 추가할 때는 결제 사용 기록을 주문과 연결하는 방법을 결정한다. 현재 `PointUsage`는 지급 건별 결제·만료 금액만 보관하고 주문 ID는 저장하지 않는다. 취소 시 실제 사용한 지급 건별 금액을 복원해야 한다면, `PAYMENT` 기록에 `orderId`를 보관하거나 같은 목적의 별도 결제 기록을 도입한다. 이는 현재 주문 취소가 범위 밖이므로 그 기능과 함께 ADR로 확정한다.
